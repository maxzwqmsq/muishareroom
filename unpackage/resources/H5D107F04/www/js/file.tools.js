...
var fileTool = (function(mui, com) {
	...
	function galleryImgs(cb) {
		...
		plus.gallery.pick(function(e) {
			...
			filter: itool.options.filter,
			multiple: itool.options.multiple,
			maximum: itool.options.maxPicsCount,
			system: false,
			onmaxed: function() {
				mui.toast('最多选择' + itool.options.maxPicsCount + '个文件');
			}
		});
	};
	...
	function processing_imgs(files, cb) {
		...
		plus.nativeUI.showWaiting('处理中，请耐心等待...');
